...
RUN export LUA_LIB_NAME=lua5.3 && cd  $(ls -1d nuster*|tail -n1) && make -j 3 TARGET=linux-glibc USE_LUA=1 LUA_INC=/usr/include/lua5.3 USE_OPENSSL=1 USE_PCRE=1 USE_ZLIB=1 && make install PREFIX=/usr/local/nuster
